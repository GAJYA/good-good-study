class Compiler {
    ...
    update (node, key, attrName) {
        ...
        if(attrName.startsWith('on')){
            return this.onUpdate(node, key, attrName)
        }
    ...
    onUpdate (node, key, attrName) {
        // v-on:click.once
        const eventName = attrName.includes(':') && attrName.split(':')[1]
        eventName = eventNameattrName.includes('.') && attrName.split('.')[1]
        const action = attrName.includes('.') && attrName.split('.')[1]
        console.log(action);
        node.addEventListener(eventName, this.vm.$options.methods[key], action? {[action]: true}: false)
    }
