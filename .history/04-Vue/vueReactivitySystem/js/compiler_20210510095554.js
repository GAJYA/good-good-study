class Compiler {
    ...
    htmlUpdate (node, key) {
        console.dir(node);
        // 1、获取到vm[key]，即要传入的html
        // 2、遍历vm[key]，以对象的方式处理所有的标签以及嵌套标签，包括tagName,textContent,childNodes,attrbutes
        // 3、根据对象的值调用appendChild方法添加节点，递归调用？？？
        Array.from(node.children).push(this.vm[key])
        new Watcher(this.vm, key, (newValue) => {
            node.textContent = newValue
        })
    }
